...
export default function MyTasks() {
  ...
  const addTask = async (e) => {
    ...
    try {
      ...
      if (!response.ok) { alert("Database Error! Your Python server might have crashed."); return; }
      setTitle(''); setDueDate(''); setPriority('Medium'); setFile(null);
      fetchTasks();
    } catch (error) {
      alert("Connection Failed! Please make sure your Python backend (app.py) is currently running.");
      console.error(error);
    }
  };
  ...
  const toggleComplete = async (id, currentStatus) => {
    try {
      await fetch(`${API_URL}/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ completed: !currentStatus }) });
      fetchTasks();
    } catch (error) { alert("Could not update task. Check backend server."); }
  };

  const deleteTask = async (id) => {
    try { await fetch(`${API_URL}/${id}`, { method: 'DELETE' }); fetchTasks(); }
    catch (error) { alert("Could not delete task. Check backend server."); }
  };
  ...
  return (
    <div>
        ...
        <form onSubmit={addTask} className="add-task-form" style={{ flexDirection: 'row', alignItems: 'flex-end', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <div className="form-group" style={{ flex: '1', minWidth: '160px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <input
                type="file"
                id="file-upload-intern"
                onChange={handleFileChange}
                style={{ display: 'none' }}
              />
              <label
                htmlFor="file-upload-intern"
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '6px',
          ...
          <button type="submit" className="submit-btn" style={{ marginTop: '0', height: '42px', alignSelf: 'flex-end' }}><Plus size={18} /> Add</button>
        </form>
      </div>
      ...
      <div className="content-grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
        <div className="card">
          <div className="card-header"><h3 style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><Clock size={18} color="#dd6b20" /> Pending ({pendingTasks.length})</h3></div>
        ...
        <div className="card">
          <div className="card-header"><h3 style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><CheckCircle size={18} color="#166534" /> Completed ({completedTasks.length})</h3></div>
          <div className="task-list" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
